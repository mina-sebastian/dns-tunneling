...
def resolve_ns(ns_domain): # resolves the domain
...
def dns_query(ns_ip, query_domain):
    try:
        ...
        logging.error(f"Could not perform the DNS query: {e}")
    return None

def dns_send_data(thread_index, ns_ip, data):
    max_label_size = 63  # the maximum size of a DNS label
    max_url_length = 250  # the maximum size of a URL
    encoded_data = base64.b32encode(data).decode('utf-8')
    # logging.info(f"{thread_index} Encoded data: {encoded_data}")

    # split the data into chunks of max_label_size
    ...
    # send the remaining chunks from the packet
    if current_query:
        current_query += f".{DNS_TUNNEL_DOMAIN}"
        response = dns_query(ns_ip, current_query)
        if response:
...
def dns_receive_data(thread_index, ns_ip):
    ...
    while not response: # keep requesting until a response is received
        response = dns_query(ns_ip, query_domain)
...
def handle_client(client_socket, thread_index):
    ...
    logging.info(f"The server {DNS_TUNNEL_DOMAIN} with IP {ns_ip}")
    try:
        # configure the client's socket to use SOCKS5
        ...
        rasp = dns_send_data(thread_index, ns_ip, f"CONNECT {addr}:{port}".encode('utf-8'))
        while not rasp:
            rasp = dns_send_data(thread_index, ns_ip, f"CONNECT {addr}:{port}".encode('utf-8'))

        if rasp != "CONNECTED":
            logging.error(f"Connection failed! Response: {rasp}")
            client_socket.close()
            return
        
        logging.info(f"{thread_index} Connected! Response: {rasp}")
        ...
        # start redirecting data between the client and the remote server through the DNS tunnel
...
def start_forwarding_threads(thread_index, client_socket, ns_ip):
    ...
    # create separate threads to read from both client and server faster
    ...
    try:
        while active_thread.get(thread_index, True):
            ...
            if client_socket in readable:
                ...
                logging.error(f"{thread_index} Received data from client: {len(data)}")
                if not data:
                    with lock:
        ...
        logging.error(f"{thread_index} Error while redirecting data from client to server: {e}")
        with lock:
            active_thread[thread_index] = False
    finally:
        client_socket.close()

def server_to_client(thread_index, client_socket, ns_ip): # read from the server through MANY DNS requests
    try:
        while active_thread.get(thread_index, True):
            logging.info(f"{thread_index} Waiting for data from server...")
            try:
                response_data = dns_receive_data(thread_index, ns_ip)
            except Exception as e:
                logging.info(f"{thread_index} Did not receive data from DNS server. {e}")
                response_data = None
        ...
        logging.error(f"{thread_index} Error while redirecting data from server to client: {e}")
        with lock:
            active_thread[thread_index] = False
    finally:
        client_socket.close()
